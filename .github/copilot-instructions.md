...
- The Firebase CLI default project in `.firebaserc` must match `src/firebase/config.ts`. When deploying outside Firebase App Hosting, add each browser-facing deployment hostname to Firebase Authentication's authorized domains for that project.
