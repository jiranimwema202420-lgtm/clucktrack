...
import { updateFlockTotals } from "@/services/flock.services";

export type ExpenditureFormData = z.infer<typeof expenditureSchema>;
...
export async function importExpenditures(
  ...
  for (const item of expenditures) {
    ...
    const newExpenditure = {
      ...item,
      expenditureDate: Timestamp.fromDate(item.expenditureDate),
      amount,
    };
